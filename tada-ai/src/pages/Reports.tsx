...
const sampleReports = [
  { id: 'r1', title: 'Q4 2025 Executive Report', dataset: 'sales_q4_2025.csv', format: 'PDF', sections: 8, created: '2025-12-01T10:00:00Z', status: 'ready', pages: 24 },
  { id: 'r2', title: 'Customer Segmentation Analysis', dataset: 'customer_segments.xlsx', format: 'Excel', sections: 6, created: '2025-11-28T15:30:00Z', status: 'ready', pages: 15 },
  { id: 'r3', title: 'Marketing Campaign Performance', dataset: 'marketing_campaign.csv', format: 'PDF', sections: 5, created: '2025-11-25T09:00:00Z', status: 'ready', pages: 18 },
]
...
export function Reports() {
  ...
  const fetchReportsAndDatasets = React.useCallback(async () => {
    try {
      ...
      if (Array.isArray(reportsRes.data)) {
        setReports(reportsRes.data.length > 0 ? reportsRes.data : sampleReports)
      }
      ...
      console.warn("Failed to fetch reports/datasets list, using fallback", err)
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.warn("Failed to generate report via API, using simulated fallback", err)
      await new Promise(r => setTimeout(r, 2000))
      const fallbackReport = {
        id: Math.random().toString(36).substr(2, 9),
        title: newReport.title,
        dataset_id: newReport.datasetId,
        dataset_name: datasetsList.find(d => d.id === newReport.datasetId)?.name || 'Dataset.csv',
        format: newReport.format,
        sections: 8,
        pages: 22,
        created_at: new Date().toISOString(),
        status: 'ready',
        download_url: '/uploads/dummy_report.pdf'
      }
      setReports(prev => [fallbackReport, ...prev])
      setShowForm(false)
      toast.success(`${newReport.title} generated (Simulation mode)`)
    } finally {
      setGenerating(false)
    }
  }
    ...
    <div className="p-6 space-y-6 max-w-[1200px] mx-auto">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="section-subheader">Generate professional PDF, Excel, and PowerPoint reports</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {(['PDF', 'Excel', 'PowerPoint'] as ReportFormat[]).map(fmt => (
