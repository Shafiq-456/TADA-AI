...
// Generate deterministic star positions for the background
...
function DemoLoginModal({ onClose, githubClientId, onGitHubSignIn }: DemoLoginModalProps) {
      ...
      >
        ...
        <div className="relative group flex justify-center mb-3">
          <button
            ...
            className={`w-full flex items-center justify-center gap-3 px-6 py-3.5 border rounded-full font-bold text-xs shadow-sm transition-colors ${
              githubClientId 
                ? 'border-slate-700 bg-[#1e293b] hover:bg-[#0f172a] text-white' 
                : 'border-slate-700/50 bg-slate-800/40 text-slate-400 cursor-not-allowed'
            }`}
          >
            <svg className={`w-4 h-4 fill-current ${!githubClientId && 'opacity-50'}`} viewBox="0 0 24 24">
          ...
          {!githubClientId && (
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 border border-slate-700 text-[10px] text-text-muted px-2.5 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap shadow-md">
              GitHub OAuth is not configured in backend .env
            </div>
          )}
        </div>
...
export function LandingPage() {
  ...
  // Fetch GitHub Client ID config from backend
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/auth/github/config`)
      .then((res) => res.json())
      .then((data) => {
        if (data.client_id) {
        ...
        >
          ...
          >
            <button
              ...
              onClick={handleGitHubSignIn}
              className="btn-primary text-base px-8 py-4 group relative overflow-hidden bg-[#24292e] hover:bg-[#1a1e22] border-white/10"
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          >
            ...
            <div className="relative">
              ...
              <button
                id="cta-github-signin-btn"
                onClick={handleGitHubSignIn}
                className="btn-primary text-base px-8 py-4 inline-flex items-center gap-3 bg-[#24292e] hover:bg-[#1a1e22] border-white/10"
                style={{ background: '#1e293b' }}
              >
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                  <path fill="currentColor" fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.579.688.481C19.137 20.162 22 16.418 22 12c0-5.523-4.477-10-10-10z"/>
