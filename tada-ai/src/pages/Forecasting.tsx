...
import { TrendingUp, RefreshCw, Settings, CheckCircle2, Target, Clock, History } from 'lucide-react'
...
export function Forecasting() {
  ...
  useEffect(() => {
    if (activeTab === 'history') {
      ...
      api.get('/api/forecasts/history')
        .then(r => setForecastHistory(Array.isArray(r.data) ? r.data : []))
        .catch(() => {})
        ...
        <div className="flex gap-2">
          <button
            ...
            className={cn('flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all',
              activeTab === 'forecast' ? 'bg-primary text-white' : 'bg-background-secondary border border-border text-text-muted hover:text-text')}
          >
            <TrendingUp size={14} /> New Forecast
          </button>
          <button
            id="tab-history"
            onClick={() => setActiveTab('history')}
            className={cn('flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all',
              activeTab === 'history' ? 'bg-primary text-white' : 'bg-background-secondary border border-border text-text-muted hover:text-text')}
          >
      ...
      {activeTab === 'history' && (
        <div className="glass-card p-5">
          <h3 className="font-semibold text-text mb-4 flex items-center gap-2">
            <Clock size={15} className="text-primary" /> Past Forecast Runs
          </h3>
          {historyLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-text-muted text-sm">No forecasts run yet. Use the <strong>New Forecast</strong> tab to get started.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-text-muted text-xs">
                    <th className="text-left py-2 pr-4 font-medium">Dataset</th>
                    <th className="text-left py-2 pr-4 font-medium">Target Column</th>
                    <th className="text-left py-2 pr-4 font-medium">Model</th>
                    <th className="text-left py-2 pr-4 font-medium">Horizon</th>
                    <th className="text-left py-2 pr-4 font-medium">Accuracy</th>
                    <th className="text-left py-2 pr-4 font-medium">R²</th>
                    <th className="text-left py-2 pr-4 font-medium">MAE</th>
                    <th className="text-left py-2 font-medium">Date</th>
                  </tr>
                </thead>
                <tbody>
                  {forecastHistory.map((f, i) => (
                    <tr key={f.id} className="border-b border-border/50 last:border-0 hover:bg-white/[0.02] transition-colors">
                      <td className="py-3 pr-4 text-text font-medium truncate max-w-[140px]">{f.dataset_name}</td>
                      <td className="py-3 pr-4 text-text-muted font-mono text-xs">{f.target_column}</td>
                      <td className="py-3 pr-4">
                        <span className="badge-primary text-[10px] capitalize">{f.model_type}</span>
                      </td>
                      <td className="py-3 pr-4 text-text-muted">{f.horizon_days}d</td>
                      <td className="py-3 pr-4">
                        <span className={cn('font-semibold', f.accuracy_score >= 85 ? 'text-success' : 'text-warning')}>
                          {f.accuracy_score != null ? `${f.accuracy_score.toFixed(1)}%` : '—'}
                        </span>
                      </td>
                      <td className="py-3 pr-4 text-text-muted">{f.r2_score != null ? f.r2_score.toFixed(3) : '—'}</td>
                      <td className="py-3 pr-4 text-text-muted">{f.mae != null ? f.mae.toFixed(2) : '—'}</td>
                      <td className="py-3 text-text-subtle text-xs">{new Date(f.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
